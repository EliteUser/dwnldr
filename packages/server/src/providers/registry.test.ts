...
import { classifySource, getProvider, getProviderByUrl, resolveSourceInfo } from './registry.js';
...
describe('provider registry', () => {
  ...
  it('rejects unsupported and malformed URLs', () => {
    expect(getProviderByUrl('https://example.com/track')).toBeNull();
  ...
  it('exposes provider capabilities', () => {
    ...
    expect(getProvider('youtube').capabilities.downloads).toBe(true);
  });
});
